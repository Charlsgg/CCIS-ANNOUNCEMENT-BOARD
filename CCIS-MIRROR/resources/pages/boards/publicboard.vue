<template>
  <div class="min-h-screen bg-black text-white p-4 md:p-8 font-sans selection:bg-orange-500/30 overflow-x-hidden relative">
    <div class="fixed top-[-10%] left-[-10%] w-[40%] h-[40%] bg-orange-900/20 rounded-full blur-[120px] pointer-events-none"></div>
    ...
    <header class="flex flex-col md:flex-row justify-between items-start mb-16 gap-8 relative z-10">
      <div class="flex items-center gap-4 animate-in fade-in slide-in-from-left duration-700 group cursor-default">
        <div class="text-orange-500 group-hover:scale-110 transition-transform duration-500">
          <svg class="w-12 h-12 filter drop-shadow-[0_0_8px_rgba(249,115,22,0.4)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="text-center md:absolute md:left-1/2 md:-translate-x-1/2 animate-in fade-in zoom-in duration-1000">
        <h1 class="text-7xl md:text-8xl font-light tracking-tighter leading-none glow-text font-mono">{{ currentTime }}</h1>
      ...
      <div class="hidden lg:block animate-in fade-in slide-in-from-right duration-700">
        <div class="glass-card p-4 w-64 border-orange-500/10">
          ...
          <div class="grid grid-cols-7 gap-1 text-center">
            ...
            <div v-for="d in daysInMonth" :key="d" :class="['p-1 text-[10px] transition-all duration-300', d === currentDay ? 'bg-orange-500 text-black rounded-full font-bold shadow-[0_0_15px_rgba(249,115,22,0.6)] scale-110' : 'opacity-60']">
              {{ d }}
            </div>
          </div>
        </div>
    ...
    <main class="max-w-6xl mx-auto relative z-10">
      <section class="flex flex-col md:flex-row break gap-4 mb-12 items-center">
        <button @click="goBack" class="flex items-center gap-2 px-6 py-3 rounded-full bg-white/5 hover:bg-orange-500/10 border border-white/10 hover:border-orange-500 transition-all duration-300 text-sm font-medium group shrink-0">
          <span class="material-symbols-outlined text-sm group-hover:-translate-x-1 transition-transform">arrow_back</span>
          <span>Back</span>
        </button>

        <div class="relative grow w-full">
          <input v-model="searchQuery" class="w-full glass-input rounded-xl px-12 py-3 text-lg hover:border-orange-500 focus:ring-1 focus:ring-orange-500 transition-all duration-300" placeholder="Search Announcements..." type="text" />
          <span class="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 opacity-40">search</span>
        </div>

        <button @click="goToEvents" class="flex items-center gap-2 px-6 py-3 rounded-full bg-white/5 hover:bg-orange-500/10 border border-white/10 hover:border-orange-500 transition-all duration-300 text-sm font-medium group shrink-0">
      ...
      <TransitionGroup name="list" tag="div" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <article v-for="(item, index) in filteredAnnouncements" :key="item.id" class="glass-card p-6 flex flex-col h-full group relative overflow-hidden" :style="{ '--delay': `${index * 0.1}sec` }">

          <div class="flex items-center gap-4 mb-6 relative z-10">
            <div class="relative">
              <img v-if="item.author_avatar" :src="item.author_avatar" class="w-12 h-12 rounded-lg object-cover shadow-lg border border-white/10 group-hover:border-orange-500/50 transition-all duration-500" />
              <div v-else :class="['w-12 h-12 rounded-lg flex items-center justify-center font-black text-xl italic transition-all duration-500 group-hover:rotate-3 shadow-lg', getPosBg(item.author_type)]">
                {{ item.author_name ? item.author_name.substring(0, 1).toUpperCase() : 'A' }}
              </div>
              <div class="absolute -bottom-1 -right-1 w-3.5 h-3.5 rounded-full border-2 border-black" :class="getPosBg(item.author_type)"></div>
            </div>

            <div>
              <h3 class="font-bold text-lg leading-tight group-hover:text-orange-400 transition-colors line-clamp-1 text-white">
                {{ item.title }}</h3>
          ...
          <div class="mt-8 flex justify-between items-center relative z-10 border-t border-white/5 pt-4">
            <button @click="handleLike(item)" class="flex items-center gap-2 transition-all duration-300 group/like relative" :disabled="item.isCooldown || item.isProcessing" :class="[ item.isAnimating ? 'text-orange-500' : 'text-white/40 grayscale', item.isCooldown && !item.isAnimating ? 'cursor-not-allowed opacity-70' : 'hover:text-orange-400' ]">
              <span class="material-symbols-outlined text-xl transition-all duration-300" :class="{ 'fill-icon scale-125 animate-pop': item.isAnimating }">
                favorite
              </span>
              <span class="text-xs font-bold">{{ item.likes_count }}</span>
              <span v-if="item.isCooldown" class="absolute -top-4 left-0 text-[8px] font-black text-orange-500 tracking-tighter animate-pulse">
                {{ item.cooldownTimer }} secs
              </span>
            </button>

            <button @click="openModal(item)" class="text-[10px] font-bold uppercase tracking-[0.2em] text-orange-500 hover:text-white transition-all flex items-center gap-2 group/btn relative z-10">
    ...
    <Teleport to="body">
      <Transition name="modal-fade">
        <div v-if="isModalOpen" class="fixed inset-0 z-100 flex items-center justify-center p-4 md:p-6">
          ...
          <div v-if="selectedAnnouncement" class="relative w-full max-w-4xl rounded-3xl border border-white/10 shadow-[0_0_50px_rgba(0,0,0,0.6)] backdrop-blur-2xl flex flex-col max-h-[90vh] overflow-hidden modal-scale" style="background: linear-gradient(145deg, rgba(24,24,27,0.85) 0%, rgba(9,9,11,0.95) 100%);">
            ...
            <div class="flex items-center justify-between px-8 py-6 border-b border-white/5 relative z-10 bg-white/2">
              ...
              <button @click="closeModal" class="text-white/40 hover:text-orange-500 hover:rotate-90 transition-all duration-300 rounded-full p-2 hover:bg-white/5 shrink-0">
            ...
            <div class="flex-1 overflow-y-auto p-8 md:p-10 custom-scrollbar relative z-10 space-y-10">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="flex items-center gap-4 bg-white/3 hover:bg-white/6 transition-colors px-5 py-4 rounded-2xl border border-white/5 group/author relative overflow-hidden">
                  ...
                  <div :class="['w-12 h-12 rounded-xl flex items-center justify-center font-bold shadow-lg text-white z-10 shrink-0 group-hover/author:scale-110 transition-transform', getPosBg(selectedAnnouncement.author_type)]">
                    <img v-if="selectedAnnouncement.author_avatar" :src="selectedAnnouncement.author_avatar" class="w-full h-full rounded-xl object-cover" />
              ...
              <div class="bg-black/20 p-6 md:p-8 rounded-2xl border border-white/5">
                ...
                <div class="prose prose-invert wrap-break-word max-w-none text-white/80 leading-relaxed text-[15px] prose-p:mb-4 prose-a:text-orange-400" v-html="selectedAnnouncement.content"></div>
              </div>
            ...
            <div class="px-8 py-5 border-t border-white/5 bg-black/40 flex justify-between items-center relative z-10 backdrop-blur-md">
              <button @click="handleLike(selectedAnnouncement)" class="flex items-center gap-2 transition-all duration-300 group/modal-like relative" :disabled="selectedAnnouncement.isCooldown || selectedAnnouncement.isProcessing" :class="[ selectedAnnouncement.isAnimating ? 'text-orange-500' : 'text-white/40 grayscale', selectedAnnouncement.isCooldown && !selectedAnnouncement.isAnimating ? 'cursor-not-allowed opacity-70' : 'hover:text-orange-400' ]">
                <span class="material-symbols-outlined text-2xl transition-all duration-300" :class="{ 'fill-icon scale-125 animate-pop': selectedAnnouncement.isAnimating }">
                  favorite
                </span>
                <span class="text-sm font-bold">{{ selectedAnnouncement.likes_count }}</span>
                <span v-if="selectedAnnouncement.isCooldown" class="absolute -top-5 left-0 text-[10px] font-black text-orange-500 tracking-tighter animate-pulse w-max">
                  {{ selectedAnnouncement.cooldownTimer }} secs
                </span>
              </button>

              <button @click="closeModal" class="px-8 py-3 rounded-xl bg-white/5 hover:bg-orange-500 hover:shadow-[0_0_20px_rgba(249,115,22,0.4)] hover:text-black transition-all duration-300 text-sm font-bold tracking-wide group flex items-center gap-2 text-white">
                <span>Close</span>
      ...
      <Transition name="fade">
        <div v-if="activePreview" class="fixed inset-0 z-110 flex items-center justify-center bg-black/95 backdrop-blur-md p-4">
          <button @click="activePreview = null" class="absolute top-6 right-6 z-120 w-12 h-12 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 text-white transition-all hover:rotate-90">
