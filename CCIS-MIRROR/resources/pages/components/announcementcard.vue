...
<template>
    <article class="flex flex-col md:flex-row overflow-hidden rounded-xl border transition-all group hover:shadow-md"
        ...
        <div v-if="coverImage" 
             class="w-full md:w-1/3 aspect-video md:aspect-auto overflow-hidden shrink-0 border-b md:border-b-0 md:border-r"
             :style="{ borderColor: surface.borderSubtle }">
            <div class="w-full h-full min-h-[200px] bg-center bg-no-repeat bg-cover group-hover:scale-105 transition-transform duration-500 cursor-pointer"
                 :style="{ backgroundImage: `url('${getFileUrl(coverImage.file_path)}')` }"
                 @click="$emit('preview', coverImage)"
        ...
        <div class="flex-1 p-5 md:p-6 flex flex-col justify-between min-w-0">
            ...
            <div>
                <div class="flex justify-between items-start mb-3">
                    ...
                    <div v-if="showActions" class="flex items-center gap-2 shrink-0 ml-4">
                        <button @click.prevent.stop="$emit('edit', post)"
                                ...
                                :style="{ backgroundColor: surface.inputBg, borderColor: surface.borderSubtle, color: theme.accent }" title="Edit">
                ...
                <h3 class="text-xl md:text-2xl font-bold mb-2 transition-colors cursor-pointer"
                    :style="{ color: isExpanded ? theme.accent : styles.textPrimary.color }"
                ...
                <div v-if="isExpanded && remainingAttachments.length > 0" class="mb-4 pt-4 border-t" :style="{ borderColor: surface.borderSubtle }">
            ...
            <div class="flex items-center justify-between mt-4 pt-4 border-t" :style="{ borderColor: surface.borderSubtle }">
                ...
                <button @click="isExpanded = !isExpanded" 
                        class="flex items-center gap-1 text-sm font-bold hover:underline shrink-0" 
